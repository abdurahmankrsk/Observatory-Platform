...
void main() {
  ...
  // Ocean / terrain transition at noise threshold 0.45
  float oceanMask = smoothstep(0.42, 0.48, terrain);

  // Final surface color
  vec3 surface = mix(uOceanColor, uTerrainColor, oceanMask);
  surface = mix(surface, uIceColor, poleBlend);
  ...
  float spec = pow(max(dot(vNormal, halfVec), 0.0), 32.0) * (1.0 - oceanMask) * 0.0;
  // Note: ocean specularity suppressed here; handled via shininess in future pass
  ...
  vec3 finalColor = surface + atmColor;
